...
%   NF2
syms kon koff kp b gamma alpha ST beta epsilon0 epsilon1 epsilon2
p =[kon; koff; kp; b; gamma; alpha; ST; beta; epsilon0; epsilon1; epsilon2];
...
num0 = epsilon0 * ( b   * ( (1 - 3*q_minus^2) * q_plus^2 + q_minus^2 + q_minus*q_plus ) + koff * ( -2*q_minus^2 * q_plus^2 + q_minus^2 + q_minus*q_plus + q_plus^2 ) );
den0 = ( q_minus^2 + q_minus*q_plus + q_plus^2 ) * ( b + koff + 2*kp + gamma * P ); 

num2 = ( kp + gamma*P ) * ( -3*q_minus^2 * q_plus^2 + q_minus^2 + q_minus*q_plus + q_plus^2 );

num3 = epsilon1 * ( kp  * ( (1 - 3*q_minus^2) * q_plus^2 + q_minus^2 + q_minus*q_plus ) - koff *   q_minus^2 * q_plus^2 );

h = [
    num0/den0 + num2/den0 + num3/den0 + q_minus^2*q_plus^2/(q_minus^2 + q_minus*q_plus + q_plus^2); 
    epsilon0*C0 + epsilon1*C1 + epsilon2*C2;
    T;
    T + C0 + C1 + C2
    ];
...
u = [];
w = [];
save('Negative_EmaxNF2','x','p','u','w','h','f','ics','known_ics');