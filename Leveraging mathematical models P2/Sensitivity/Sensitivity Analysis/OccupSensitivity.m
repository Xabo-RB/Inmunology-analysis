...
for i = 1:length(koffVect)
    ...
    p = complex([5e-5, koffVect(i)], 0);

    solution = sensitivity(x0, p, d, tspan);

    % COJO LA RESPUESTA QUE ME INTERESA:
    SolResponse = solution{1}(:, 3); 
    % Normalización de la respuesta
    newSol = abs((SolResponse .* koffVect(i)) ./ solution{1}(:, 1)); 
...
set(gca, 'YDir', 'normal', 'FontSize', 14, 'YScale', 'log');
hold on
...
%     % Normalización de la respuesta
%     newSol = abs((SolResponse .* konVect(i)) ./ solution{1}(:, 1)); 
% 
%     % En la fila que define un valor de koff
%     results_matrix(i, :) = newSol;
% end
...
% title('Occup.', 'FontSize', 18, 'FontWeight', 'bold', 'Color', 'k');
% set(gca, 'YDir', 'normal');
% hold on

%save('Occ_kp.mat','tspan','koffVect','results_matrix');
...
function solution = sensitivity(x0, p, d, tspan)
    ...
    ST = @(t,y)ODEOccupancy(t, y, p);
    options = odeset('RelTol',1e-4,'AbsTol',1e-4, 'Refine', 1);
...
function dx = ODEOccupancy(t, x, p)
    dx = zeros(3, 1);
