...
ode = @ODEmodel(
    ...
    C2'(t) = kp*C1(t) - koff*C2(t) + kon*P(t)*Tast(t),
    Tast'(t) = koff*C2(t) - kon*P(t)*Tast(t) - lambda*Tast(t),
    y1(t) = (lambda * (kon * (T(t) + C0(t) + C1(t) + C2(t) + C3(t) + Tast(t)) + 2*lambda) + (kon * (T(t) + C0(t) + C1(t) + C2(t) + C3(t) + Tast(t)) - 2*lambda) * (
        (
            (1 - 2*(kp / (kp + koff))^2)^2 * lambda^2 +
            koff * (kp / (kp + koff))^2 * (
                koff * (kp / (kp + koff))^2 +
                2*lambda * (1 + 2*(kp / (kp + koff))^2)
            )
        )^(1/2) +
        (kp / (kp + koff))^2 * (2*lambda + koff) )) / (4 * kon * lambda)
)
